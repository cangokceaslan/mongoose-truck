...
export class MongooseTruck {
    ...
    async runMultipleDbMethodology() {
        ...
        for (let i = 1; i < this.froms?.length; i++) {
            ...
            await this.runDbMethodology(i);
            //Logs.log(this.froms?.length)
        }
    ...
    async runDbMethodology(page = 0) {
        ...
        for (let i = 0; i < pages; i++) {
            ...
            let data = await this.ModelFrom.find(this.from?.query)
                .populate(this.from?.populate)
                .skip(this.from?.skip + (i * this.from?.concurrent))
                .sort(this.from?.sort)
                .select(this.from?.select)
                .limit(this.from?.concurrent)
                .lean();
            if (data.length === 0) {
                break;
            }
            ...
            try {
                await Promise.all(this.ModelTo.map((ModelTo, index) => {
                    return ModelTo.insertMany(data, { ordered: false }).then(res => {
